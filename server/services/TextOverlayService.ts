...
export class TextOverlayService {
  ...
  async generateRealGraphic(productName: string, platform: string): Promise<any> {
    try {
      console.log('🎨 Generating real AI graphic for platform:', platform);
      ...
      if (!candidates || candidates.length === 0) {
        console.warn('⚠️ No image generated by AI, falling back to SVG');
        return this.generateFallbackSVG(productName, platform);
      }

      const content = candidates[0].content;
      if (!content || !content.parts) {
        console.warn('⚠️ No content parts in AI response, falling back to SVG');
        return this.generateFallbackSVG(productName, platform);
      }
      ...
      for (const part of content.parts) {
        if (part.text) {
          ...
          console.log(`📸 Real graphic saved as ${filename}`);
          imageGenerated = true;
          break;
        }
      }
      ...
      if (!imageGenerated) {
        console.warn('⚠️ No image data received from AI, falling back to SVG');
        return this.generateFallbackSVG(productName, platform);
      }
  ...
  private generateFallbackSVG(productName: string, platform: string): any {
    try {
      console.log('🎨 Generating fallback SVG graphic for:', productName);
      ...
      console.log(`📸 Fallback SVG saved as ${filename}`);
